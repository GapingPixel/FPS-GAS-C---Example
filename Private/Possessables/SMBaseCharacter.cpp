...
ASMBaseCharacter::ASMBaseCharacter(const class FObjectInitializer& ObjectInitializer) :
	...
	{
	HealthComp = CreateDefaultSubobject<USMHealthComponent>(TEXT("HealthComp"));
...
void ASMBaseCharacter::ApplyPersonalSpacePush()
{
	...
	for (ASMBaseCharacter* character : OverlappingCharacters)
	{
		if (character)
		{
...
void ASMBaseCharacter::OnCurrentEquippableChanged(ASMEquippableBase* OldEquippable)
{
	...
	if (ASMEquippableBase* equippable = InventoryComponent->GetCurrentEquippable())
	{
...
void ASMBaseCharacter::SetupGas()
{
	...
	if (SMPlayerStateCache)
	{
		...
		if (!ASC)
		{
			UE_LOG(LogSpawnMaster, Error, TEXT("AbilitySystemComponent in %s is nullptr. Did the PlayerState setup its ASC correctly? Server/Client: %i"), *GetNameSafe(this), (int32)HasAuthority())
		}
	...
	{
		UE_LOG(LogSpawnMaster, Error, TEXT("Player State cast failed in %s in SetupGas(). Server/Client: %i"), *GetNameSafe(this), (int32)HasAuthority())
	}
...
void ASMBaseCharacter::SetupAttributes()
{
	if (USMAbilitySystemComponent* ASC = GetSMAbilitySystemComponent())
	{
		...
		FGameplayEffectSpecHandle NewHandle = ASC->MakeOutgoingSpec(StartingAttributes, 1, EffectContextHandle);
...
void ASMBaseCharacter::AddDefaultAbilities(USMAbilitySystemComponent* ASC, TArray<FAbilityClassWithLevel>& AbilitiesToAdd)
{
	...
	for (FAbilityClassWithLevel& abilityInfo : AbilitiesToAdd)
	{
