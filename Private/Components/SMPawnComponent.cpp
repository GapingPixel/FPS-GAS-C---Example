...
void USMPawnComponent::OnRegister()
{
	...
	if (pawn && pawn->Implements<UAbilitySystemComponent>())
	{
		...
		if (interface)
		{
			...
			if (!asc)
			{
				UE_LOG(LogSpawnMaster, Error, TEXT("Ability System Component could not be found in OnRegister in %s"), *GetNameSafe(this))
				return;
			}
		...
		{
			UE_LOG(LogSpawnMaster, Error, TEXT("Ability System Interface could not be found in OnRegister in %s"), *GetNameSafe(this))
		}
	}
}
...
void USMPawnComponent::InitializeAbilitySystemComponent(USMAbilitySystemComponent* InASC, AActor* InOwnerActor)
{
	...
	UE_LOG(LogSpawnMaster, Verbose, TEXT("Setting up ASC [%s] on pawn [%s] owner [%s], existing [%s] "), *GetNameSafe(InASC), *GetNameSafe(Pawn), *GetNameSafe(InOwnerActor), *GetNameSafe(ExistingAvatar));
